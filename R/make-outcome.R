...
dh.makeOutcome <- function(
  ...
  mult_vals = NULL, remove_temp = TRUE, cohorts = names(opals), keep_original = FALSE, 
  df_name = NULL) {
  ...
  if(remove_temp == TRUE){
    ...
    end_objs <- ds.ls(datasources = opals[cohorts])
    ...
    dh.tidyEnv(obj = to_remove, type = "remove", cohorts = cohorts)
