...
![plot of chunk plot-weight-age](figure/plot-weight-age-1.png)
...
```
## Error: `new_data` must not be NULL.
```
...
## ℹ Input `..1` is `cohort == coh_ref & between(age, min, max)`.
## x Input `..1` must be of size 502 or 1, not size 0.
```
...
```
## Error in is.data.frame(y): object 'pred_by_age' not found
```
